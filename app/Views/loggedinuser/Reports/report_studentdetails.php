                            ...
                            <tbody>
                                ...
                                ?>
                                        <tr>
                                            ...
                                            <td>
                                                <b>Committed Fee:</b> <?php 
                                                if($result->TotalFeesIncludingDue != '' || $result->TotalFeesIncludingDue != NULL)
                                                {
                                                    echo $result->TotalFeesIncludingDue;
                                                }else
                                                {
                                                echo $result->TotalFeesToPay ; } ?><br />
                                                <b>Paid Fee:</b> <?php 
                                                $helperModel = new HelperModel();
                                                $promoted = $helperModel->studentcount($result->userid,$result->batchid);
                                                $myString =  $result->applicationnumber ;
                                                 $promoted1 = $helperModel->studentbatchcount($result->userid);
                                               if($promoted1==0)
                                               {
                                                   $newfee = $result->TotalFeesPaid-2500;
                                                    echo $newfee.".00";
                                               }else{
                                                if ($myString > 2300001){
                                                    $newfee = $result->TotalFeesPaid-2500;
                                                ...
                                                {
                                                    echo $result->TotalFeesPaid;
                                                }
                                               }
                                                 ?><br />
                                                
                                                <b>Balance Amount:</b> <?php 
                                                 if($promoted1==0)
                                               {
                                                   $newfee = $result->BalanceToBePaid+2500+$result->FeesDue;
                                                    echo $newfee.".00";
                                               }else{
                                                 if ($myString > 2300001){
                                                    $newremain = $result->BalanceToBePaid+2500+$result->FeesDue;
                                                    echo $newremain.".00";
                                                }
                                                elseif ($myString > 220001 && $promoted==1) {
                                                    $newremain =  $result->BalanceToBePaid+2500+$result->FeesDue;
                                                ...
                                                {
                                                echo $result->BalanceToBePaid+$result->FeesDue;
                                                }
                                               }
                                                ?><br />
                                            </td>
...
<script>
    function filter(download = 0) {
        var URL = "<?php echo base_url('reports/report_studentdetails') ?>" + "?" + $('#filterForm').serialize();
